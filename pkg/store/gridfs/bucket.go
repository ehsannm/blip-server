...
type DecryptFunc func([]byte) []byte
...
func NewBucket(db *mongo.Database, encryptFunc EncryptFunc, decryptFunc DecryptFunc, opts ...*options.BucketOptions) (*Bucket, error) {
	b := &Bucket{
		name:        "fs",
...
func (b *Bucket) OpenUploadStreamWithID(fileID interface{}, filename string, opts ...*options.UploadOptions) (*UploadStream, error) {
	...
	return newUploadStream(upload, b.encryptFunc, fileID, filename, b.chunksColl, b.filesColl), nil
}
...
func (b *Bucket) OpenDownloadStreamByName(filename string, opts ...*options.NameOptions) (*DownloadStream, error) {
	...
	findOpts := options.Find().SetSkip(int64(numSkip)).SetSort(bsonx.Doc{{Key: "uploadDate", Value: bsonx.Int32(sortOrder)}})

	return b.openDownloadStream(bsonx.Doc{{Key: "filename", Value: bsonx.String(filename)}}, findOpts)
}
...
func (b *Bucket) Delete(fileID interface{}) error {
	...
	res, err := b.filesColl.DeleteOne(ctx, bsonx.Doc{{Key: "_id", Value: id}})
...
func (b *Bucket) Rename(fileID interface{}, newFilename string) error {
	...
	res, err := b.filesColl.UpdateOne(ctx,
		bsonx.Doc{{Key: "_id", Value: id}},
		bsonx.Doc{{Key: "$set", Value: bsonx.Document(bsonx.Doc{{Key: "filename", Value: bsonx.String(newFilename)}})}},
	)
	if err != nil {
		return err
	}
...
func (b *Bucket) openDownloadStream(filter interface{}, opts ...*options.FindOptions) (*DownloadStream, error) {
	...
	fileEncryptedElem := cursor.Current.Lookup("encrypted")
	decryptFunc := func(b []byte) []byte {
		return b
	}
	switch fileEncryptedElem.Type {
	case bsontype.Boolean:
		if fileEncryptedElem.Boolean() {
	...
	if fileLen == 0 {
		return newDownloadStream(nil, decryptFunc, b.chunkSize, 0), nil
	}
	...
	return newDownloadStream(chunksCursor, decryptFunc, b.chunkSize, int64(fileLen)), nil
}
...
func (b *Bucket) deleteChunks(ctx context.Context, fileID interface{}) error {
	...
	_, err = b.chunksColl.DeleteMany(ctx, bsonx.Doc{{Key: "files_id", Value: id}})
	return err
}
...
func (b *Bucket) findChunks(ctx context.Context, fileID interface{}) (*mongo.Cursor, error) {
	...
	chunksCursor, err := b.chunksColl.Find(ctx,
		bsonx.Doc{{Key: "files_id", Value: id}},
		options.Find().SetSort(bsonx.Doc{{Key: "n", Value: bsonx.Int32(1)}})) // sort by chunk index
	if err != nil {
		return nil, err
	}
...
func (b *Bucket) createIndexes(ctx context.Context) error {
	...
	docRes := cloned.FindOne(ctx, bsonx.Doc{}, options.FindOne().SetProjection(bsonx.Doc{{Key: "_id", Value: bsonx.Int32(1)}}))
	...
	filesModel := mongo.IndexModel{
		Keys: bson.D{
			{Key: "filename", Value: int32(1)},
			{Key: "uploadDate", Value: int32(1)},
		},
	}

	chunksModel := mongo.IndexModel{
		Keys: bson.D{
			{Key: "files_id", Value: int32(1)},
			{Key: "n", Value: int32(1)},
		},
